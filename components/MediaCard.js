...
class MediaCard extends Component {
  ...
  handleDelete() {
    ...
    if (!web3) {
      return alert(
        'No injected web3 instance was detected - Please install e.g. MetaMask'
      )
    }
    ...
    web3.personal.sign(web3.toHex(id), web3.eth.defaultAccount, (err, res) => {
      if (res) {
        fetch(
          location.origin + `/api/delete?videoId=${id}&signedMsg=${res}`
        ).then(function(res) {
          res.ok
            ? alert('Succesfully deleted video')
            : alert('Something went wrong with deleting your video')
        })
      }
    })
  }
    ...
    return (
      ...
      >
        ...
        <Box p={3}>
          ...
          {wallet && (
            <Items>
              <span className="fa fa-address-card" aria-hidden="true" /> WALLET:{' '}
              {wallet}
            </Items>
          )}
          <Box mt={3}>
            ...
            <Flex mt={2} mb={2}>
              ...
              >
                <Button
                  ...
                  color="#2c0d54"
                  bgcolor="white"
                  data-clipboard-text={location.origin + '/view/' + id}
                >
